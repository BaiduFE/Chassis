$(document).ready(function() {
  ...
  test("clear attributes", 1, function() {
    var Model = Chassis.Model.extend({defaults:{a:1}});
    var model = new Model({a: 2});
    model.clear();
  ...
  test("underscore methods", 5, function() {
    var model = new Chassis.Model({ 'foo': 'a', 'bar': 'b', 'baz': 'c' });
  ...
  test("clone", 10, function() {
    var a = new Chassis.Model({ 'foo': 1, 'bar': 2, 'baz': 3});
    var b = a.clone();
    equal(a.get('foo'), 1);
    equal(a.get('bar'), 2);
    equal(a.get('baz'), 3);
    ...
    var foo = new Chassis.Model({p: 1});
    var bar = new Chassis.Model({p: 2});
  ...
  test("isNew", 6, function() {
    ...
    a = new Chassis.Model({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': -5 });
    ok(!a.isNew(), "any defined ID is legal, negative or positive");
    a = new Chassis.Model({ 'foo': 1, 'bar': 2, 'baz': 3, 'id': 0 });
    ok(!a.isNew(), "any defined ID is legal, including zero");
    ok( new Chassis.Model({          }).isNew(), "is true when there is no id");
    ok(!new Chassis.Model({ 'id': 2  }).isNew(), "is false for a positive integer");
    ok(!new Chassis.Model({ 'id': -5 }).isNew(), "is false for a negative integer");
  });
  ...
  test("set and unset", 8, function() {
    var a = new Chassis.Model({id: 'id', foo: 1, bar: 2, baz: 3});
    var changeCount = 0;
  ...
  test("set falsy values in the correct order", 2, function() {
    var model = new Chassis.Model({result: 'result'});
    model.on('change', function() {
  ...
  test("multiple unsets", 1, function() {
    ...
    var model = new Chassis.Model({a: 1});
  ...
  test("using a non-default id attribute.", 5, function() {
    var MongoModel = Chassis.Model.extend({idAttribute : '_id'});
  ...
  test("set an empty string", 1, function() {
    var model = new Chassis.Model({name : "Model"});
    model.set({name : ''});
  ...
  test("clear", 3, function() {
    var changed;
    var model = new Chassis.Model({id: 1, name : "Model"});
  ...
  test("defaults", 4, function() {
    var Defaulted = Chassis.Model.extend({
      defaults: {
        "one": 1,
        "two": 2
      }
    ...
    Defaulted = Chassis.Model.extend({
      defaults: function() {
        return {
          "one": 3,
          "two": 4
  ...
  test("change, hasChanged, changedAttributes, previous, previousAttributes", 9, function() {
    var model = new Chassis.Model({name: "Tim", age: 10});
  ...
  test("changedAttributes", 3, function() {
    var model = new Chassis.Model({a: 'a', b: 'b'});
    deepEqual(model.changedAttributes(), false);
  ...
  test("change with options", 2, function() {
    var value;
    var model = new Chassis.Model({name: 'Rob'});
  ...
  test("change after initialize", 1, function () {
    ...
    var obj = new Chassis.Model(attrs);
  ...
  test("save within change event", 1, function () {
    var env = this;
    var model = new Chassis.Model({firstName : "Taylor", lastName: "Swift"});
    model.url = '/test';
    model.on('change', function () {
      model.save();
  ...
  test("validate after save", 2, function() {
    var lastError, model = new Chassis.Model();
  ...
  test("destroy", 3, function() {
    ...
    var newModel = new Chassis.Model;
  ...
  test("validate on unset and clear", 6, function() {
    var error;
    var model = new Chassis.Model({name: "One"});
    model.validate = function(attrs) {
      if (!attrs.name) {
        error = true;
        return "No thanks.";
  ...
  test("Inherit class properties", 6, function() {
    var Parent = Chassis.Model.extend({
  ...
  test("hasChanged/set should use same comparison", 2, function() {
    var changed = 0, model = new Chassis.Model({a: null});
    model.on('change', function() {
  ...
  test("set same value does not trigger change", 0, function() {
    var model = new Chassis.Model({x: 1});
  ...
  test("unset does not fire a change for undefined attributes", 0, function() {
    var model = new Chassis.Model({x: undefined});
  ...
  test("`previous` for falsey keys", 2, function() {
    var model = new Chassis.Model({0: true, '': true});
  ...
  test("`save` with `wait` sends correct attributes", 5, function() {
    var changed = 0;
    var model = new Chassis.Model({x: 1, y: 2});
    model.url = '/test';
  ...
  test("#1412 - Trigger 'sync' event.", 3, function() {
    var model = new Chassis.Model({id: 1});
  ...
  test("#1943 change calculations should use _.isEqual", function() {
    var model = new Chassis.Model({a: {key: 'value'}});
  ...
  test("isValid", function() {
    var model = new Chassis.Model({valid: true});
    model.validate = function(attrs) {
