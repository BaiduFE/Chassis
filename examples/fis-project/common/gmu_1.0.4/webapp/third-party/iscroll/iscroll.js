...
(function (window, doc) {
    ...
    var _tmpNodeArr = [], _bindArr = [];

    function creElm(tagName) {
        var node = document.createElement(tagName || 'div');
        _tmpNodeArr.push(node);
        return node;
    }

    var m = Math,
        dummyStyle = creElm().style,
        vendor = (function () {
    ...
    iScroll.prototype = {
            ...
            if (!that.moved) {
                if (hasTouch) {
                    if (that.doubleTapTimer && that.options.zoom) {
                        ...
                        that.isStopScrollAction || (that.doubleTapTimer = setTimeout(function () {
                            that.doubleTapTimer = null;
        ...
        _momentum:function (dist, time, maxDistUpper, maxDistLower, size) {
            var deceleration = 0.0006,
                speed = m.abs(dist) * this.options.speedScale / time,
        ...
        destroy:function () {
            ...
            //清除所有创建的dom
            for (var i = 0, ci; ci = _tmpNodeArr.pop();) {
                ci.parentNode && ci.parentNode.removeChild(ci);
                ci = null;
            }
            //清除所有绑定的事件
            ...
            div.innerHTML = '';
            this.wrapper = this.scroller = div = null;
        },
